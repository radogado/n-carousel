...
// Define translations and initWithTranslations before the main carousel code
let translations = {};
let translationsLoaded = false;

window.initWithTranslations = function(customTranslations) {
...
(function () {
  ...
  const updateCarousel = (el, forced = false) => {
    ...
    if (old_active_slide && !forced) {
      ...
      old_active_slide.setAttribute("aria-selected", "false");
    ...
    active_slide.setAttribute("aria-selected", "true");
  ...
  const updateAriaLabels = (el) => {
    ...
    if (index) {
      ...
      index.querySelectorAll("button").forEach((tab, i) => {
        const total = index.querySelectorAll("button").length;
        // Navigation buttons always use 'item' because they're actions
    ...
    if (content) {
      content.querySelectorAll(":scope > *").forEach((slide, i) => {
        ...
        // Slides always use 'current_item' because they're descriptions
    ...
    const close_modal = el.querySelector(".n-carousel__close button");
    if (close_modal) {
      close_modal.setAttribute("aria-label", translate('close'));
    }
    
    const full_screen = el.querySelector(".n-carousel__full-screen button");
    if (full_screen) {
      full_screen.setAttribute("aria-label", translate('fullscreen'));
    }
  };
  ...
  const init = (host = document) => {
    ...
    if (lang === 'en') {
      // For English, use built-in translations
      translations = {
        en: {
          carousel: 'Carousel',
      ...
      // Update aria-labels for all carousels
      host.querySelectorAll(".n-carousel").forEach(updateAriaLabels);
    } else {
      ...
      fetch('translations.json')
        ...
        .catch(error => {
          ...
          translationsLoaded = true; // Mark as loaded even on error to prevent infinite waiting
        });
    }
    ...
    host.querySelectorAll(".n-carousel:not([data-ready])").forEach((el) => {
      ...
      // Add ARIA attributes after core functionality is set up
      ...
      if (index) {
        ...
        index.querySelectorAll(":scope > *").forEach((tab, i) => {
          ...
          tab.setAttribute("aria-selected", tab.hasAttribute("aria-current"));
        });
      }
      if (close_modal) {
        close_modal.setAttribute("role", "button");
      }
      if (full_screen) {
        full_screen.setAttribute("role", "button");
      }
      ...
      // Update aria-labels after roles are set
      updateAriaLabels(el);
...
//# sourceMappingURL=n-carousel.rollup.js.map
