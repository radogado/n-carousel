      ...
      host.querySelectorAll(".n-carousel:not([data-ready])").forEach((el) => {
          ...
          const inView = target => {
            const interSecObs = new IntersectionObserver(entries => {
              ...
              { threshold: .99, root: target.parentElement }); // .99 works for all, including vertical auto height
          interSecObs.observe(target);
          // console.log('intersection observing ', target)
        }; targets.forEach(inView);

      });
  }; window.nCarouselInit = init; window.addEventListener('popstate', e => { // Hash navigation support
    // console.log(e);
    if (!!location.hash) {
